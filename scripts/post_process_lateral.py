        ...
        with dai.Device(p) as device:
            ...
            for file in files:
                if file.endswith('.png') and file.removesuffix('_cut.png')+'.json' in files:
                    ...
                    vector['sex'] = sex
                    vector['bag'] = int(bool(bag))
                    vector['hat'] = int(bool(hat))
                    vector['sleeves'] = int(bool(sleeves))
                    vector['pants'] = int(bool(pants))
                    vector['hair'] = int(bool(hair))
                    vector['coat'] = int(bool(coat))
                    ...
                    db.people.update_one({'embed':vector['embed']})
            # group_json_to_csv()
        ...
        with dai.Device(p) as device:
            ...
            for file in files:
                if file.endswith('.png') and file.removesuffix('_cut.png')+'.json' in files:
                    ...
                    vector['type'] = form
                    vector['postprocess'] = True
                    db.vehicles.update_one({'id': vector['id']})
            group_json_to_csv()
