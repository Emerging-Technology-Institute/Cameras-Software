...
LAT = 0
LON = 0
ORIENT = int(open('orient.txt','r').read())
MTR_TO_DEG = 111139
...
def create_pipeline():
    ...
    body_nn.setBlobPath(str(blobconverter.from_zoo("person-detection-0200", shaves=5, version=openvinoVersion)))
    ...
    rec_nn.setBlobPath(
        str(blobconverter.from_zoo('person-reidentification-retail-0277', shaves=6, version=openvinoVersion)))
...
device = dai.Device(create_pipeline(), usb2Mode=True)
                ...
                for i,t in enumerate(trackletsData):
                    ...
                    if len(msgs['recognition']) != 0:
                        ...
                        try:
                            ...
                            database_count,database_rows = model.pull_identities(db, 'person')
                            for file in database_rows:
                                # One-liner used to open a JSON file to compare against the current embedding
                                data = np.array(file['embed'])
                            ...
                            if identities[t.id] == 'unknown':
                                angle = np.arcsin((t.spatialCoordinates.x / 1000) / max(t.spatialCoordinates.z / 1000, 0.001))
                                dist = t.spatialCoordinates.z / 1000 / MTR_TO_DEG
                                xdisp = dist * np.cos(angle)
                                ydisp = dist * np.sin(angle)
                                model.det_to_json(db,
                                                  label='person',
                                                  id=f'ID_{database_count}',
                                                  embed=score,
						  ...
						  time=dt.datetime.now().strftime("%Y%m%d-%H%M%S"),
                                ...
                                cv2.imwrite(
                                    f'{str(pathlib.PurePath(os.getcwd()))}/vectors/ID_{database_count}_cut.png',
                                    frame[y1:y2,x1:x2])
                        except IndexError:
                            print("Recognition missed, likely desynchronized")
                    if identities[t.id] == target:
                        det_color = target_color
                    cv2.putText(frame, str(identities[t.id]), (x1 + 10, y1 + 20), cv2.FONT_HERSHEY_TRIPLEX, 0.5, 255)
